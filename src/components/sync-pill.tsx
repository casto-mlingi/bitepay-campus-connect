...
export function SyncPill({ className = "" }: { className?: string }) {
  const { sync } = useStore();
  const merging = sync.status === "merging";
  const busy = sync.status === "pulling" || sync.status === "pushing" || merging;

  const tone = sync.status === "error"
    ? "bg-red-100 text-red-700"
    : sync.status === "offline"
      ? "bg-amber-100 text-amber-700"
      : sync.pendingPush || merging
        ? "bg-slate-100 text-slate-600"
        : "bg-emerald-100 text-emerald-700";

  const label = sync.status === "error" ? "Sync error"
  ...
  const Icon = merging ? GitMerge : sync.status === "error" ? AlertTriangle : sync.status === "offline" ? CloudOff : Cloud;
