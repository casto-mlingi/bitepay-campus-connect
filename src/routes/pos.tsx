...
import { Search, User, Wallet, Plus, Minus, Trash2, Printer, ArrowUpCircle, CheckCircle2, X } from "lucide-react";
import { useStore, formatTZS, type Product, type Profile } from "@/lib/store";
...
export const Route = createFileRoute("/pos")({
  component: POS,
  head: () => ({ meta: [{ title: "Walk-in POS — BitePay Staff" }, { name: "description", content: "Point of sale for walk-in wallet purchases and cash top-ups." }] }),
});

type Line = { product: Product; qty: number };

function POS() {
  const { currentUser, products, findCustomer, posSale, topUp, profiles } = useStore();
  ...
  // Keep customer in sync when their balance changes (posSale / topUp update profiles).
            ...
            {categories.map((c) => (
              <button
                ...
                className={`px-4 py-1.5 rounded-full text-sm font-medium whitespace-nowrap border ${
                  category === c ? "bg-foreground text-background border-foreground" : "bg-surface border-border"
                }`}
              >{c}</button>
            ))}
          </div>
          ...
          <form onSubmit={doSearch} className="mt-3">
            <div className="relative">
              ...
              <Input
                ...
                placeholder="Search customer by phone or ID"
                className="pl-9"
              />
            </div>
            ...
            <div className="flex gap-2">
              <div className="flex-1 flex items-center border rounded-lg px-3">
                ...
                <input
                  type="number"
                  value={topupAmt}
                  onChange={(e) => setTopupAmt(Number(e.target.value) || 0)}
